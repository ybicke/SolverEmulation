...
parser.add_argument('--heights', type=list, default=None, help='heights')
...
def main():
    ...
    # Load heights from NC file
    logger.info('Loading vertical heights from NC file...')
    try:
        # Try to find NC files in the dataset directory
        nc_files = glob.glob(join(args.dataset, '*.nc'))
        if not nc_files:
            # Try parent directory
            nc_files = glob.glob(join(dirname(args.dataset), '*.nc'))
        
        if nc_files:
            # Find files that match the pattern for vertical heights
            height_file = None
            for nc_file in nc_files:
                if 'ml_ecrad' in nc_file and '70lev' in nc_file:
                    height_file = nc_file
                    break
            
            if height_file:
                logger.info(f'Found height file: {height_file}')
                # Import the function from your GNN module
                sys.path.append(dirname(dirname(__file__)))
                from column_files.gnn_graphCast_multiMesh import load_vertical_heights
                
                # Load the heights
                heights = load_vertical_heights(height_file)
                logger.info(f'Loaded heights with shape: {heights.shape}')
                
                # Store for later use in creating edge features
                args.heights = heights
                args.edge_channels_in = 1
            else:
                logger.warning('No suitable height NC file found. Using default edge features.')
                args.edge_channels_in = 0
                args.heights = None
        else:
            logger.warning('No NC files found. Using default edge features.')
            args.edge_channels_in = 0
            args.heights = None
    except Exception as e:
        logger.error(f'Error loading heights: {e}')
        args.edge_channels_in = 0
        args.heights = None
