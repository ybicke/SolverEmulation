...
print(f"Using mean of all {y_true_list[0].shape[0]} samples")
...
def plot_differences(fig, subplot_idx, x_values, y_true, y_preds, models, title, xlabel, ylabel):
    ...
    # Calculate the mean of vertical differences for ground truth across all samples
    delta_y_true_all = np.diff(y_true, axis=1)  # Calculate diffs along vertical axis
    delta_y_true_mean = np.mean(delta_y_true_all, axis=0)  # Mean across all samples
    ...
    for i, (y_pred, model) in enumerate(zip(y_preds, models)):
        ...
        # Calculate the mean of vertical differences for predictions across all samples
        delta_y_pred_all = np.diff(y_pred, axis=1)  # Calculate diffs along vertical axis
        delta_y_pred_mean = np.mean(delta_y_pred_all, axis=0)  # Mean across all samples
        ax.plot(delta_y_pred_mean, x_diff, linestyle='-', color=color, linewidth=1.5, label=f'{model["name"]}', alpha=0.7)
    
    # Plot ground truth differences
    ax.plot(delta_y_true_mean, x_diff, linestyle=':', color='red', linewidth=2, label='True')
...
plt.suptitle(f"Mean Vertical Differences Comparison (Levels {min_level}-{max_level-1})", fontsize=16)

# Save the figure
plt.savefig(f'/mydata/deepcloud/yves/Vertical_differences_testSet2_GNN_HRLU_Models_levels_{min_level}-{max_level-1}.png', bbox_inches='tight', dpi=300)
print(f"Figure saved as Vertical_differences_testSet2_GNN_HRLU_Models_levels_{min_level}-{max_level-1}.png") 