...
def plot_differences(fig, subplot_idx, x_values, y_true, y_preds, models, title, xlabel, ylabel):
    ...
    # First convert to numpy if it's a torch tensor
    if isinstance(y_true, torch.Tensor):
        y_true = y_true.detach().cpu().numpy()
    
    # First compute mean over all samples, then calculate vertical differences
    y_true_mean = np.mean(y_true, axis=0)  # Mean across all samples
    delta_y_true = np.diff(y_true_mean)  # Then calculate vertical differences
    ...
    for i, (y_pred, model) in enumerate(zip(y_preds, models)):
        ...
        # First compute mean over all samples, then calculate vertical differences
        y_pred_mean = np.mean(y_pred, axis=0)  # Mean across all samples
        delta_y_pred = np.diff(y_pred_mean)  # Then calculate vertical differences
...
plt.suptitle(f"Mean-then-Difference Vertical Comparison (Levels {min_level}-{max_level-1})", fontsize=16)

# Save the figure
plt.savefig(f'/mydata/deepcloud/yves/Vertical_differences_testMean_GNN_HRLU_Models_levels_{min_level}-{max_level-1}.png', bbox_inches='tight', dpi=300)
print(f"Figure saved as Vertical_differences_testMean_GNN_HRLU_Models_levels_{min_level}-{max_level-1}.png") 