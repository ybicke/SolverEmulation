...
def plot_sorted_magnitudes(all_magnitudes_sorted, lambda_1, lambda_2, lambda_5, lambda_10, test_path):
    ...
    plt.axhline(y=lambda_1, color="red", linestyle="--", label=f"1% Quantile: {lambda_1:.6f}")
    plt.axhline(y=lambda_2, color="green", linestyle="--", label=f"2% Quantile: {lambda_2:.6f}")
    plt.axhline(y=lambda_5, color="orange", linestyle="--", label=f"5% Quantile: {lambda_5:.6f}")
    plt.axhline(y=lambda_10, color="purple", linestyle="--", label=f"10% Quantile: {lambda_10:.6f}")

    # Add labels and title
    plt.title("Sorted Fourier Magnitudes with Lower Quantile Cutoffs", fontsize=14)
    plt.xlabel("Index (Fourier Components)", fontsize=12)
    plt.ylabel("Magnitude", fontsize=12)
    plt.legend(fontsize=10)
    plt.grid(True, alpha=0.3)

    # Save the plot to the test_path directory
    plot_file = os.path.join(test_path, 'quantile_plot_full_new.png')
    plt.savefig(plot_file, dpi=300, bbox_inches='tight')
    print(f"Full plot saved to {plot_file}")
    plt.close()
    
def plot_sorted_magnitudes_zoom(all_magnitudes_sorted, lambda_values, test_path, specified_values_quantiles):
    """
    Plot the sorted Fourier magnitudes with a zoomed-in view focusing on the lower quantiles.
    ...
    # Plot only the lower portion of the data for better visibility
    lower_idx = int(len(all_magnitudes_sorted) * 0.2)  # View lowest 20% of values
    plt.plot(all_magnitudes_sorted[:lower_idx], label="Sorted Fourier Magnitudes", color="blue", alpha=0.7)
    ...
    # Add horizontal lines for specified values if they're in the visible range
    for value, quantile in specified_values_quantiles.items():
        if value <= max(all_magnitudes_sorted[:lower_idx]) * 1.1:
            plt.axhline(y=value, color="gray", linestyle=":", 
                        label=f"Value {value} (Quantile {quantile:.2%})")
    
    # Add labels and title
    plt.title("Zoomed View of Lower Fourier Magnitude Quantiles", fontsize=14)
    plt.xlabel("Index (Fourier Components)", fontsize=12)
    plt.ylabel("Magnitude", fontsize=12)
    plt.legend(fontsize=10)
    plt.grid(True, alpha=0.3)
    
    # Set y-axis limit to focus on the important range
    max_lambda = max(lambda_1, lambda_2, lambda_5, lambda_10)
    plt.ylim([0, max_lambda * 1.5])
    
    # Save the zoomed-in plot
    zoomed_plot_file = os.path.join(test_path, 'quantile_plot_zoomed_new.png')
...
def test_model(model, test_set):
    ...
    # Compute quantiles to determine thresholds
    lambda_1 = np.percentile(all_magnitudes_sorted, 1)
    lambda_2 = np.percentile(all_magnitudes_sorted, 2)
    lambda_5 = np.percentile(all_magnitudes_sorted, 5)
    lambda_10 = np.percentile(all_magnitudes_sorted, 10)
    ...
    lambda_values = {
        ...
        'lambda_5': float(lambda_5),
        'lambda_10': float(lambda_10),
    }
    ...
    # Calculate quantiles for specific values of interest
    ...
    # Create both plots
    plot_sorted_magnitudes(all_magnitudes_sorted, lambda_1, lambda_2, lambda_5, lambda_10, test_path)
