...
def plot_histograms(train_files, num_samples=500000, output_dir="histograms"):
    ...
    for i, (x3d, x2d, y) in enumerate(data_loader):
        ...
        top_flux = y[:, -3:, :]  # Shape: [batch_size, 5, 4]
        flux_values_list.append(top_flux)
    ...
    flux_values = torch.cat(flux_values_list, dim=0)  # Shape: [total_samples, 5, 4]
    print(f"flux_values shape: {flux_values.shape}")  # Should output [total_samples, 5, 4]

    # Reshape flux_values to [total_samples * 5, 4]
    flux_values_flat = flux_values.view(-1, 4)  # Shape: [N, 4]

    # Create a mask to filter out zero values only
    non_zero_mask_flat = (flux_values_flat != 0)  # Shape: [N, 4]

    # Initialize lists to store mean and std for each flux variable
    mean = []
    std = []

    # Calculate mean and std across all valid values for each flux variable
    for i in range(4):
        flux_i = flux_values_flat[:, i]
        mask_i = non_zero_mask_flat[:, i]
        valid_flux_i = flux_i[mask_i]

        if valid_flux_i.numel() == 0:
            print(f"No valid samples for flux variable {i}. Skipping mean and std calculation.")
            mean.append(torch.tensor(0.0))
            std.append(torch.tensor(1.0))
            continue

        mean_i = torch.mean(valid_flux_i)
        std_i = torch.std(valid_flux_i, unbiased=True)

        # Handle case where std is zero to avoid division by zero
        if std_i == 0:
            print(f"Standard deviation for flux variable {i} is zero. Setting std to 1.")
            std_i = torch.tensor(1.0)

        mean.append(mean_i)
        std.append(std_i)

    mean = torch.stack(mean)  # Shape: [4]
    std = torch.stack(std)    # Shape: [4]
    print(f"mean: {mean}, std: {std}")  

    # Normalize flux_values_flat using the computed mean and std, keeping zero entries as zero
    normalized_flux_values_flat = torch.where(
        non_zero_mask_flat,
        (flux_values_flat - mean) / std,
        torch.tensor(0.0, device=flux_values_flat.device)
    )
    ...
    for i in range(4):  # Only loop over the 4 flux variables
        flux_i = normalized_flux_values_flat[:, i]
        ...
        flux_i_numpy = valid_flux_i.cpu().numpy()

        plt.figure(figsize=(8, 6))
        plt.hist(flux_i_numpy, bins=bins[i], alpha=0.7, color=colors[i], edgecolor='black')
        plt.title(f'Histogram of {flux_labels[i]} Flux Values (Top 3 Levels, Normalized)')
        plt.xlabel('Normalized Flux Value')
        plt.ylabel('Frequency')
        plt.grid(True)
        plt.tight_layout()

        histogram_path = os.path.join(output_dir, f'histogram_{flux_labels[i].replace(" ", "_").lower()}_normalized_top3.png')
        plt.savefig(histogram_path)
        plt.close()
        print(f'Histogram for {flux_labels[i]} flux (normalized, top 3 levels) saved as {histogram_path}')
