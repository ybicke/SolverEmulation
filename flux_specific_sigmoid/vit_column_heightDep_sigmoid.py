...
from FluxSpecificSigmoid_diffNorm import HeightDependentSigmoid, MultimodalSigmoid
...
class ViT(nn.Module):
    ...
    def forward(self, x3d, x2d):
        ...
        x_out[:, :, remaining_fluxes] = torch.sigmoid(x_out[:, :, remaining_fluxes])
        

        # Scale the output and return
        x = self._scale_output(x_out, x2d_org)
