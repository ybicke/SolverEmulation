...
def precompute_train_target_mean_var(train_set):
    """
    Compute mean and variance of target variables from actual training data
    for both baseline comparison and metrics calculation.
    
    Returns:
        train_target_mean: Mean of the target variables (C,)
        train_target_var: Variance of the target variables (C,)
    """
    ...
    train_target_mean = torch.mean(train_targets, dim=0)  # Mean along batch dimension (C,)
    
    # Second pass to compute the variance
    squared_diff_sum = torch.zeros_like(train_target_mean)
    total_samples = 0
    
    for _, _, batch_y, _ in train_set:
        batch_y_cpu = batch_y.cpu()
        diff = batch_y_cpu - train_target_mean.unsqueeze(0)  # Expand mean for broadcasting
        squared_diff_sum += torch.sum(diff ** 2, dim=0)
        total_samples += batch_y_cpu.shape[0]
    
    train_target_var = squared_diff_sum / total_samples
    
    # Log some stats about the computed values
    mean_var = torch.mean(train_target_var).item()
    logger.info(f'Computed train target mean and variance from actual training data')
    logger.info(f'Mean variance across all features: {mean_var:.8f}')
    
    return train_target_mean, train_target_var


def train_model(model, train_set, valid_set, target_means, target_vars, train_target_mean=None):
...
def main():
    ...
    if args.train:
        ...
        # Compute train target mean and variance for baseline and evaluation metrics
        train_target_mean_file = join(test_path, 'train_target_mean.pickle')
        train_target_var_file = join(test_path, 'train_target_var.pickle')
        print('Computing train statistics from training data...')
        train_target_mean, train_target_var = precompute_train_target_mean_var(train_loader)
        
        # Save train_target_mean for backward compatibility
        ...
        # Save train_target_var for advanced evaluation metrics
        with open(train_target_var_file, 'wb') as handle:
            pickle.dump(train_target_var, handle, protocol=pickle.HIGHEST_PROTOCOL)
        print(f'Saved train_target_var to {train_target_var_file}')
    ...
    if args.test:
        ...
        test_model(model, test_loader, target_means, target_vars)
