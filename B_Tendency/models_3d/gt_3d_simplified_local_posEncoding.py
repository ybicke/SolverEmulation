...
class LocalPositionalEmbedding3D(nn.Module):
    """
    Local positional embedding for hybrid 3D attention pattern.
    
    Instead of global positional embeddings for all ~70K nodes, this creates:
    1. Vertical positional embeddings for height levels (0 to num_height_levels-1)
    2. Relative horizontal positional embeddings for k-hop neighborhoods
    
    This dramatically reduces parameters while providing better spatial inductive bias.
    """
    def __init__(self, embed_dim, num_height_levels, max_k_hop):
        super().__init__()
        self.embed_dim = embed_dim
        self.num_height_levels = num_height_levels
        self.max_k_hop = max_k_hop
        
        # Vertical positional embedding (similar to ViT-1D)
        self.vertical_pos_embed = nn.Parameter(
            torch.randn(num_height_levels, embed_dim) * 0.02
        )
        
        # Relative horizontal positional embedding
        # For k-hop neighborhoods: distance can be 0 (self), 1, 2, ..., max_k_hop
        # Plus a special token for "no connection" (boundary cases)
        max_distance = max_k_hop + 1  # 0, 1, 2, ..., k
        self.horizontal_pos_embed = nn.Parameter(
            torch.randn(max_distance + 1, embed_dim) * 0.02  # +1 for boundary token
        )
        self.boundary_token_idx = max_distance  # Index for boundary/no-connection token
        
    def compute_relative_distances(self, neighborhoods, num_columns, num_height_levels, device):
        """
        Compute relative distances for k-hop neighborhoods with boundary handling.
        
        Returns:
            relative_distances: [num_nodes, max_neighbors] distance indices
            distance_mask: [num_nodes, max_neighbors] valid distance mask
        """
        max_neighbors = max(len(neighbors) for neighbors in neighborhoods.values())
        num_nodes = num_columns * num_height_levels
        
        relative_distances = torch.full(
            (num_nodes, max_neighbors), 
            self.boundary_token_idx,  # Default to boundary token
            dtype=torch.long, 
            device=device
        )
        distance_mask = torch.zeros(num_nodes, max_neighbors, dtype=torch.bool, device=device)
        
        for node_id in range(num_nodes):
            col_id = node_id // num_height_levels
            height_level = node_id % num_height_levels
            
            neighbors = neighborhoods.get(node_id, [])
            if not neighbors:
                continue
                
            for i, neighbor_id in enumerate(neighbors):
                neighbor_col = neighbor_id // num_height_levels
                neighbor_height = neighbor_id % num_height_levels
                
                # Compute distance based on type of connection
                if col_id == neighbor_col:
                    # Same column (vertical connection) - distance 0
                    distance = 0
                else:
                    # Different column (horizontal connection at same height)
                    # Use simple Manhattan distance on grid (approximation)
                    # In practice, you'd use actual grid topology
                    col_distance = abs(col_id - neighbor_col)
                    distance = min(col_distance, self.max_k_hop)
                
                relative_distances[node_id, i] = distance
                distance_mask[node_id, i] = True
        
        return relative_distances, distance_mask
    
    def forward(self, x, neighborhoods, num_columns, num_height_levels, cached_distances=None):
        """
        Apply local positional embeddings to node features.
        
        Args:
            x: [batch, num_nodes, embed_dim] node features
            neighborhoods: dict mapping node_id -> neighbor_list
            num_columns: number of columns in the grid
            num_height_levels: number of height levels
            cached_distances: cached distance computations for efficiency
        
        Returns:
            x_with_pos: [batch, num_nodes, embed_dim] features with positional encoding
        """
        B, N, D = x.shape
        device = x.device
        
        # Compute or use cached relative distances
        if cached_distances is None:
            relative_distances, distance_mask = self.compute_relative_distances(
                neighborhoods, num_columns, num_height_levels, device
            )
        else:
            relative_distances, distance_mask = cached_distances
        
        # Apply vertical positional embeddings
        vertical_pos = torch.zeros_like(x)  # [B, N, D]
        for node_id in range(N):
            height_level = node_id % num_height_levels
            vertical_pos[:, node_id, :] = self.vertical_pos_embed[height_level]
        
        # Apply relative horizontal positional embeddings
        # This is more complex because each node has different neighbors
        horizontal_pos = torch.zeros_like(x)  # [B, N, D]
        ...
        for node_id in range(N):
            ...
            node_distances = relative_distances[node_id, :len(neighbors)]
            node_mask = distance_mask[node_id, :len(neighbors)]
            
            # Average the positional embeddings of all neighbors
            # (weighted by attention later, this is just initial bias)
            valid_distances = node_distances[node_mask]
            if len(valid_distances) > 0:
                neighbor_pos_embeds = self.horizontal_pos_embed[valid_distances]
                avg_horizontal_pos = neighbor_pos_embeds.mean(dim=0)
                horizontal_pos[:, node_id, :] = avg_horizontal_pos
        
        # Combine vertical and horizontal positional embeddings
        pos_embedding = vertical_pos + horizontal_pos
...
class SimplifiedNeighborhoodAttention(nn.Module):
    ...
    def get_simplified_neighborhoods_with_boundary_handling(self, edge_index, num_nodes, num_columns, num_height_levels, cached_neighborhoods=None):
        """
        Compute hybrid neighborhoods with proper boundary handling.
        
        Boundary cases:
        1. Corner columns: fewer horizontal neighbors
        2. Edge columns: reduced k-hop reach
        3. Interior columns: full k-hop neighborhoods
        
        For each node, the neighborhood includes:
        1. All nodes in the same column (full vertical attention)
        2. Same-height nodes in k-hop neighboring columns (with boundary awareness)
        ...
        for i in range(edge_index.size(1)): # Loop through all edges
        ...
        # For each column, find k-hop neighboring columns with boundary awareness
        column_neighborhoods = {}
        for col_id in range(num_columns):
            current_hop = {col_id}
            all_neighbor_cols = {col_id}
            
            # Iteratively expand k-hop neighborhoods
            for hop in range(self.max_hops):
                next_hop = set()
                for col in current_hop:
                    # Only add neighbors that actually exist in the graph
                    available_neighbors = adj_list[col]
                    next_hop.update(available_neighbors)
                
                # Remove already visited columns
                current_hop = next_hop - all_neighbor_cols
                all_neighbor_cols.update(current_hop)
                
                # Stop if no new neighbors found (boundary reached)
                if not current_hop:
                    break
    ...
    def forward(self, x, edge_index, num_columns, shared_cache=None):
        ...
        if shared_cache is not None:
            if shared_cache['neighborhoods'] is None:
                ...
                neighborhoods = self.get_simplified_neighborhoods_with_boundary_handling(
                    single_batch_edges, N_per_batch, num_columns, num_height_levels
                ) # each node received its neighborhood list
...
class SimplifiedGraphTransformer3D(nn.Module):
    ...
    def __init__(self,
        ...
        # Local positional embeddings (dramatically reduced parameters)
        self.local_pos_embedding = LocalPositionalEmbedding3D(
            embed_dim=embed_dim,
        ...
        # Shared cache for efficiency (including distance computations)
        self._shared_cache = {
            'neighborhoods': None,
            'attention_mask': None,
            'distance_cache': None
    ...
    def forward(self, x3d_norm, x2d_norm):
        ...
        # Cache distance computations for efficiency
        if self._shared_cache['distance_cache'] is None and self._shared_cache['neighborhoods'] is not None:
            neighborhoods = self._shared_cache['neighborhoods']
            relative_distances, distance_mask = self.local_pos_embedding.compute_relative_distances(
                neighborhoods, num_columns, self.num_height_levels, self.device
            )
            self._shared_cache['distance_cache'] = (relative_distances, distance_mask)
        
        # Apply local positional embeddings
        if self._shared_cache['neighborhoods'] is not None:
            x_flat = self.local_pos_embedding(
                x_flat, 
                self._shared_cache['neighborhoods'], 
                num_columns, 
                self.num_height_levels,
                self._shared_cache['distance_cache']
            )
