...
class Attention(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, dropout=0.):
        ...
        # Linear layer to project the input to query, key, and value matrices
        self.to_qkv = nn.Linear(dim, inner_dim * 3, bias=False)

        # If project_out is True, it consists of a linear layer and a dropout layer
        # If project_out is False, it is an identity function
...
class ViT(nn.Module):
    """
        Vision Transformer (ViT) for ICON.

        Args:
            num_cells (int): Number of cells in the input.
            patch_size (int): Size of each patch.
            dim (int): Dimension of the input and output embeddings.
            mlp_dim (int): Dimension of the feed-forward network.
            depth (int): Number of transformer layers.
            heads (int): Number of attention heads.
            ...
            swflx_idx (list): Indices for shortwave flux.
            lwflx_idx (list): Indices for longwave flux.
            cosmu0_idx (int): Index for cosine of solar zenith angle.
            tsfctrad_idx (int): Index for surface temperature.
            mean2d (torch.Tensor): Mean for 2D normalization.
            var2d (torch.Tensor): Variance for 2D normalization.
            mean3d (torch.Tensor): Mean for 3D normalization.
            var3d (torch.Tensor): Variance for 3D normalization.
    ...
    def __init__(
        ...
        # Rearrange the input tensor to create patches
        # Input patch_size: (batch_size, num_cells, height, channels_in)
        # Rearrange: (batch_size, num_patches, patch_size, height, channels_in)
        # --> here num_patches = num_cells // patch_size, num_cells dim. split into num_patches of size patch_size
        # Flatten: (batch_size, num_patches, patch_dim)
        # --> Collapse the last three dimensions into one, here patch_dim = patch_size * height * channels_in
        # nn.Linear, projects the normalized patches into a higher-dimensional space.
    ...
    def forward(self, x3d, x2d):
        """
        Args:
            x3d (torch.Tensor): 3D input tensor.
            x2d (torch.Tensor): 2D input tensor.

        Returns:
            torch.Tensor: Output tensor after passing through the Vision Transformer.
        """
        if self.is_test:
            pass
        ...
        # Pass through the MLP head and apply sigmoid
        x = self.mlp_head(x)
        x = self.sigmoid(x)
        
        # Scale the output and return
...
class ViT4(ViT):
    """ Similar to ViT, just separate embedings for 3d and 2d features """
    def __init__(
            self,
            num_cells,
            patch_size,
        ...
        super().__init__(num_cells, patch_size, dim, channels_in=channels_in, 
                         channels_out=channels_out, height=height, device=device,
                         *args, **kwargs)

        # Define the patch embedding layer for 2D features
        # Uses separate patch embedding layers for 3D and 2D inputs.
    ...
    def forward(self, x3d, x2d):
        ...
        #print(f"Initial x3d shape: {x3d.shape}")
        #print(f"Initial x2d shape: {x2d.shape}")

        x3d = self.normalizer3d(x3d)
        x2d_org = x2d.clone()
        x2d = self.normalizer2d(x2d)
        #print(f"Normalized x3d shape: {x3d.shape}")
        #print(f"Normalized x2d shape: {x2d.shape}")
        #print(f"Normalized x2d_org shape: {x2d_org.shape}")

        # Apply patch embedding to the 3D an 2D input
        x3d = self.to_patch_embedding(x3d)
        x2d = self.to_patch_embedding_2D(x2d)
        #print(f"After to_patch_embedding x3d shape: {x3d.shape}")
        #print(f"After to_patch_embedding x2d shape: {x2d.shape}")

        # Concatenate the embedded 3D and 2D inputs
        x = torch.cat((x3d, x2d[:, None, :]), axis=-2)
        ...
        x += self.pos_embedding[:, :self.num_patches]
        x = self.dropout(x)
        #print(f"After positional embedding and dropout shape: {x.shape}")
        ...
        x = self.mlp_head(x)
        x = self.sigmoid(x)
        #print(f"After MLP head and sigmoid shape: {x.shape}")

        # Scale the output and return
        x = self._scale_output(x, x2d_org)
        #print(f"Scale output shape: {x.shape}")
        x =  x.squeeze()
        #print(f"Final output shape: {x.shape}")
        return x
