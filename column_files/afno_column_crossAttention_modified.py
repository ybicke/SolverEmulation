...
class Block(nn.Module):
    def __init__(self, 
        ...
        # AFNO1D layer for atmospheric embeddings
        if mixing_type == "afno":
...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, atmos_emb, surface_emb):
        ...
        atmos_emb = atmos_emb + self.dropout(attn_output)
        return atmos_emb
...
class AFNONet(nn.Module):
    ...
    def __init__(self, 
        ...
        # With uniform false and drop_path_rate to 0 not really used. Responsible for calculating the drop path rates for each 
    ...
    def forward(self, x3d, x2d):
        ...
        x3d_dummy = self.to_patch_embedding(self.dummy_vector)
        ...
        x3d_dummy_across_batch = x3d_dummy.repeat(x3d.shape[0], 1, 1)
        x3d = torch.cat((x3d, x3d_dummy_across_batch), dim=1)
