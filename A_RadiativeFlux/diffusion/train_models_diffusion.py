...
# Add these arguments to your parser arguments section
diffusion_group = parser.add_argument_group('Diffusion model arguments')
diffusion_group.add_argument('--num-sampling-steps', type=int, default=25, 
                           help='Number of sampling steps for diffusion model')
diffusion_group.add_argument('--deterministic-sampling', action=argparse.BooleanOptionalAction, default=True, 
                           help='Whether to use deterministic sampling in diffusion model')
diffusion_group.add_argument('--sigma-min', type=float, default=0.002, 
                           help='Minimum noise level for diffusion model')
diffusion_group.add_argument('--sigma-max', type=float, default=80.0, 
                           help='Maximum noise level for diffusion model')
diffusion_group.add_argument('--sigma-data', type=float, default=0.5, 
...
def get_model(model_name):
    ...
    if model_name == 'vit':
        ...
        from models.diffusion import RadiativeDiffusionModel
        
        model = RadiativeDiffusionModel(
            height_in=args.height_in,
            channel_3d=args.channel_3d,
            channel_2d=args.channel_2d,
            channel_out=args.channel_out,
...
def train_model(model, train_set, valid_set, normalizer):
    ...
    # Check if we're using a diffusion model
    is_diffusion = args.model == 'diffusion'
    ...
    for epoch in range(init_epoch, args.num_epoch):
        ...
        for i, data in enumerate(train_set):
            ...
            # Handle diffusion model differently
            if is_diffusion:
                # Diffusion model training step
                loss = model.training_step(batch_x3_norm, batch_x2_norm, batch_x2_orig)
                
                # Get output for metrics calculation
                with torch.no_grad():
                    # For metrics, we use the direct UNet output without diffusion
                    outputs = model.unet(batch_x3_norm, batch_x2_norm, batch_x2_orig)
                    mse_loss = train_loss(outputs, batch_y)
                    batch_mae = train_mae(outputs, batch_y)
                    
                    # Calculate heating rate smoothness if enabled
                    if use_hr_smoothness:
            ...
            if i > 0:
                ...
                if i % 100 == 99:
                    if is_diffusion:
                        print(f'batch {i+1}, time:{t2_1-t1_1:.3f}, diffusion_loss: {loss:.4f}, '
                              f'mse: {mse_loss:.4f}, mean_absolute_error: {batch_mae:.4f}')
                    elif use_hr_smoothness:
        ...
        with torch.no_grad():
            for i, v_data in enumerate(valid_set):
                ...
                if is_diffusion:
                    # For validation, we use the UNet directly without diffusion
                    v_outputs = model.unet(vx3d_norm, vx2d_norm, vx2d_orig)
                else:
                    # Standard model forward pass
...
def test_model(model, test_set, normalizer):
    ...
    for i, data in enumerate(test_set):
        ...
        with torch.no_grad():
            ...
            if is_diffusion:
                # For diffusion model, sample from the diffusion process
                outputs = model.sample(batch_x3_norm, batch_x2_norm, batch_x2_orig)
            else:
