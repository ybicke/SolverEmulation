#!/usr/bin/env python3
"""
Clean unified training script for radiative flux prediction that supports:
...
Usage:
    ...
    python unified_train_flux_clean.py --model vit --mode 1d --dataset-type full
    python unified_train_flux_clean.py --model gnn --mode 1d --dataset-type triangle --triangle-id 0
    
    # 3D models (triangle-wise)  
    python unified_train_flux_clean.py --model gnn_3d --mode 3d --dataset-type triangle --grid-file-path /path/to/grid.nc
    
    # With heating rate smoothness loss
    python unified_train_flux_clean.py --model vit --mode 1d --hr-smoothness-weight 0.1 --hr-smoothness-top-levels 10
"""
...
from unified_flux_data_loader import UnifiedFluxDataset
from data_utils import DataNormalizer
from flux_utils import calculate_heating_rates, HeatingRateSmoothnessLoss
from evaluation_utils import process_timing_statistics, warm_up_model
...
parser.add_argument('--model', type=str, required=True, help='Model type: vit, afno, gnn, gnn_3d, rnn, unet, etc.')
parser.add_argument('--mode', type=str, default='1d', choices=['1d', '3d'], help='Training mode: 1d (column-wise) or 3d (triangle-wise)')
parser.add_argument('--dataset-type', type=str, default='triangle', choices=['triangle', 'full'], help='Dataset selection: triangle (specific region) or full (all columns)')
...
parser.add_argument('--patch-size', type=int, default=2, help='Patch size (ViT/AFNO)')
...
parser.add_argument('--hr-smoothness-weight', type=float, default=0.0, help='Weight for heating rate smoothness loss (0.0 to disable)')
parser.add_argument('--hr-smoothness-top-levels', type=int, default=None, help='Apply heating rate smoothness only to top N levels')
...
def get_model():
    ...
    # 3D Models    
        ...
        elif args.model == 'structured_transformer':
            from files_3d.transformer_3d_structured import StructuredTransformer3D
            model = StructuredTransformer3D(
                total_cols=args.num_cells,
                grid_file_path=args.grid_file_path,
                triangle_id=args.triangle_id,
                embed_dim=args.hidden_dim,
...
def setup_training_components(model, args, device):
    """Setup optimizer, metrics, and loss functions for training"""
    # Setup optimizer
    ...
    hr_smoothness = None
    if args.hr_smoothness_weight > 0:
    ...
    return optimizer, (train_loss, valid_loss, train_mae, valid_mae), hr_smoothness


def run_training_epoch(model, train_loader, optimizer, metrics, hr_smoothness, normalizer, args, device):
    """Run one training epoch"""
    train_loss, _, train_mae, _ = metrics
    ...
    return train_loss.compute(), train_mae.compute(), smoothness_losses


def run_validation_epoch(model, valid_loader, metrics, hr_smoothness, normalizer, device):
    """Run one validation epoch"""
    _, valid_loss, _, valid_mae = metrics
    ...
    with torch.no_grad():
        for i, v_data in enumerate(valid_loader):
            ...
            if hr_smoothness is not None:
                ...
                smoothness_losses.append(v_smoothness_loss)
            
            valid_loss.update(v_outputs, v_labels)
            valid_mae.update(v_outputs, v_labels)
    
    return valid_loss.compute(), valid_mae.compute(), smoothness_losses


def train_model(model, train_set, valid_set, normalizer):
    """Main training function with clean epoch separation"""
    ...
    optimizer, metrics, hr_smoothness = setup_training_components(model, args, device)
    ...
    for epoch in range(init_epoch, args.num_epoch):
        ...
        # Training epoch
        total_train_loss, total_train_mae, smoothness_losses_train = run_training_epoch(
            model, train_set, optimizer, metrics, hr_smoothness, normalizer, args, device
        )
        
        # Validation epoch
        total_valid_loss, total_valid_mae, smoothness_losses_valid = run_validation_epoch(
            model, valid_set, metrics, hr_smoothness, normalizer, device
        )
        ...
        if hr_smoothness is not None and smoothness_losses_train:
            log_dict['hr_smoothness'] = sum(smoothness_losses_train) / len(smoothness_losses_train)
        if hr_smoothness is not None and smoothness_losses_valid:
...
def setup_testing_components(args, device):
    """Setup metrics and loss functions for testing"""
    test_loss = MeanSquaredError().to(device)
    test_mae = MeanAbsoluteError().to(device)
    
    # Setup smoothness loss if enabled
    hr_smoothness = None
    if args.hr_smoothness_weight > 0:
        hr_smoothness = HeatingRateSmoothnessLoss(
            weight=1.0,
...
def run_testing_loop(model, test_loader, normalizer, test_loss, test_mae, hr_smoothness, args, device):
    """Run the main testing loop with timing measurements"""
    # Timing configuration
    num_timing_batches = 5 if args.mode == '3d' else 100
    timing_data = []
    
    # Result collection
    y_true, y_pred, h_true, h_pred = [], [], [], []
    smoothness_losses = []
    ...
    with torch.no_grad():
        for i, data in enumerate(test_loader):
            ...
            # Calculate metrics
            test_loss.update(outputs, batch_y)
            test_mae.update(outputs, batch_y)
            ...
            if i % 100 == 99:
                print(f'batch {i+1}, loss: {test_loss.compute():.4f}, mae: {test_mae.compute():.4f}')
    ...
    results = {
        ...
        'test_mae': total_test_mae,
        'smoothness_losses': smoothness_losses
    }

    return timing_data, results


def save_test_results(results, timing_data, test_path, model, args):
    """Save test results and timing statistics"""
    # Print final metrics
    print(f'Final Test Loss: {results["test_loss"]:.6f}, Test MAE: {results["test_mae"]:.6f}')
    
    if results['smoothness_losses']:
        avg_smoothness = sum(results['smoothness_losses']) / len(results['smoothness_losses'])
        print(f'HR smoothness: {avg_smoothness:.4f}')
        wandb.log({"test_hr_smoothness": avg_smoothness})

    # Save prediction results
    result_files = ['y_true', 'y_pred', 'h_true', 'h_pred']
    for key in result_files:
        with open(join(test_path, f'{key}.pickle'), 'wb') as f:
            pickle.dump(results[key], f, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    if timing_data:
        timing_stats = process_timing_statistics(
            ...
            test_loss=results['test_loss'], test_mae=results['test_mae']
        )
        wandb.log({
...
def test_model(model, test_set, normalizer):
    """Main testing function with clean component separation"""
    logger.info('Testing started...')

    # Load best model
    load_best_model(model, checkpoint_path, device)

    # Setup testing components
    test_loss, test_mae, hr_smoothness = setup_testing_components(args, device)
    ...
    # Run testing loop
    timing_data, results = run_testing_loop(
        model, test_set, normalizer, test_loss, test_mae, hr_smoothness, args, device
    )
    
    # Save results
    save_test_results(results, timing_data, test_path, model, args)
