...
gnn_3d_group.add_argument('--disable-horizontal', action=argparse.BooleanOptionalAction, default=False, help='Disable horizontal edges')
...
def train_model(model, train_set, valid_set, normalizer):
    ...
    for epoch in range(init_epoch, args.num_epoch):
        ...
        print(f'{epoch_number:03}/{args.num_epoch}: ',
            f'time: {t2-t1:.3f}, valid_time: {t_valid_end-t_valid_start:.3f}', 
...
def calculate_heating_rates(y, x3d, x2d): 
    ...
    # Handle 3D data with column dimension (batch, columns, levels, features)
    if len(x3d.shape) == 4:
    ...
    # Handle original single-column data (batch, levels, features)
    else:
        qv = x3d[..., qv_ecrad_in_idx]
...
def main():
    ...
    if args.train:
        ...
        val_loader = get_column_data_with_disk_cache(val_files, 
                                                     shuffle=False,
                                                     triangle_id=args.triangle_id,
                                                     division_factor=args.triangle_division_factor,
                                                     num_workers=args.num_workers
