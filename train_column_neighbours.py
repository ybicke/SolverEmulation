...
parser.add_argument('--neighborhood-size', type=int, default=1, help='Neighborhood size')
...
def get_column_data_with_disk_cache(filenames, neighborhoods, subsample=args.subsample, shuffle=False, num_workers=0):
    icon_data = IconColumnIterableDataset(filenames, neighborhoods, subsample=subsample, cache_dir='/tmp', shuffle=shuffle)
...
def main():
    ...
    if args.train:
        ...
        train_loader = get_column_data_with_disk_cache(train_files, neighborhoods, shuffle=True)
        val_loader = get_column_data_with_disk_cache(train_files, neighborhoods, shuffle = False, subsample=1.0)
    ...
    if args.test:
        test_loader = get_column_data_with_disk_cache(test_files, neighborhoods, shuffle=False)
