...
def test_model(model, test_set):
    ...
    # Initialize attention matrices list for each layer
    attention_matrices = [[] for _ in range(args.vit_layers)]
    
    # ... existing code above ...
    # Let's say we only want single-head plots for layers 1 and 3:
    selected_head_layers = [3]
    ...
    for i, data in enumerate(test_set):
        ...
        model.eval()
        with torch.no_grad():
            # Forward pass (optional if needed for something else)
            outputs = model(batch_x3, batch_x2)

            # Go through ALL layers for average attention
            for layer_idx in range(args.vit_layers):
                attention_weights = model.get_attention_weights(batch_x3, batch_x2, layer_idx=layer_idx)
                # attention_weights => [batch_size, num_heads, seq_len, seq_len]

                # (A) Average across heads => shape [batch_size, seq_len, seq_len]
                averaged_over_heads = attention_weights.mean(dim=1)
                # Then average across batch => shape [seq_len, seq_len]
                avg_attention_matrix = averaged_over_heads.mean(dim=0)

                logger.info(f"Plotting average attention for layer {layer_idx} ...")
                model.visualize_attention(
                    avg_attention_matrix.cpu(),
                    layer_idx=layer_idx,
                    save_path=test_path
                )

                # (B) Plot single heads only if layer_idx is in our 'selected_head_layers'
                if layer_idx in selected_head_layers:
                    num_heads = attention_weights.shape[1]
                    for head_idx in range(num_heads):
                        # Extract single head => [batch_size, seq_len, seq_len]
                        single_head_attn = attention_weights[:, head_idx, :, :]
                        # Average over the batch => shape [seq_len, seq_len]
                        single_head_avg = single_head_attn.mean(dim=0)

                        logger.info(f"Plotting single-head attention for layer {layer_idx}, head {head_idx} ...")
                        # Pass a custom string so we know which head/layer in filename
                        model.visualize_attention(
                            single_head_avg.cpu(),
                            layer_idx=f"{layer_idx}_head{head_idx}",
                            save_path=test_path
                        )

            logger.info("Finished plotting attention for the first batch.")
    ...
    ## Visualize attention for each layer AFTER the test loop is finished 
    #logger.info('Generating attention visualizations...')
    #for layer_idx in range(args.vit_layers):
    #    model.visualize_attention(test_set, layer_idx) # Call visualize_attention here
